	...
	SEMICOLON.slider = {
					...
					swiperSlider = new Swiper(element.find('.swiper-parent'), {
						...
						on: {
							...
							transitionEnd: function(swiper) {
								...
								if (
									element.find('.swiper-slide.swiper-slide-active').find('video').length > 0 &&
									sliderVideoAutoPlay == true
								) {
		...
		captionPosition: function() {
			$sliderElement.find('.slider-caption:not(.custom-caption-pos)').each(function() {
				...
				if (
					$(this).parents('#slider').prev('#header').hasClass('transparent-header') &&
					($body.hasClass('device-xl') || $body.hasClass('device-lg'))
				) {
							...
							$(this).toggleClass('acctitlec').next().stop().slideDown('normal', function() {
								if (
									...
									$('html,body').stop(true).animate(
										{
											scrollTop:
												clickTarget.offset().top - (SEMICOLON.initialize.topScrollOffset() - 40)
										},
										800,
										'easeOutQuad'
									);
		...
		twitterFeed: function() {
			...
			if ($twitterFeedEl.length > 0) {
				$twitterFeedEl.each(function() {
					...
					$.getJSON(
						twitterFeedLoader + '?username=' + twitterFeedUser + '&count=' + twitterFeedCount,
						function(tweets) {
